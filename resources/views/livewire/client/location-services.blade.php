        ...
        <div class="items-center px-8 py-12 mx-auto max-w-7xl lg:px-16 md:px-12 2xl:py-5 relative">
            <div class="justify-center w-full text-center lg:p-10 max-auto relative">
                <div class="justify-center w-full mx-auto">
                    ...
                    <p class="mt-8 2xl:text-5xl text-3xl font-bold tracking-tighter text-white">
                        ({{ $category_name }})
                    </p>
    ...
    <section class="mx-auto max-w-7xl py-10 relative">
        ...
        <div class="mt-5 grid grid-cols-1 2xl:grid-cols-4 gap-5">
            @forelse ($services as $item)
                <div class="border bg-white hover:shadow-main shadow-sm hover:shadow-lg rounded-xl ">
                    <div class="flex flex-col">
                        <img src="{{ Storage::url($item->image_path) }}" class="h-56 object-cover rounded-xl m-2"
                            alt="">
                        <div class="p-3">
                            <p class="font-semibold text-main uppercase text-lg text-center">
                                {{ $item->name }}
                            </p>
                            <p class="font-semibold text-main  text-sm text-center">
                                ({{ $item->service_provider->user->name }})
                            <h1 class="text-sm text-center text-red-400">
                                {{ $item->service_provider->location->location }}</h1>
                            </p>
                            ...
                            <div class="price mt-5">
                                <div class="flex space-x-2 justify-center">
                                    ...
                                    <span class="text-gray-600 font-medium">&#8369;{{ number_format($item->price, 2) }}
                                        @if ($item->is_hour)
                                            /hour
                                        @endif
                                    </span>
                            ...
                            <div class="my-3 mt-5  flex justify-center items-center">
                                <x-button label="BOOK NOW"
                                    href="{{ route('client.services-provider', ['id' => $item->id]) }}" rounded outline
                                    teal right-icon="arrow-right" class="font-semibold" />
                            </div>
                        </div>
                    </div>
                </div>
