<div x-data="{ info: @entangle('has_details') }">
    <section class="relative
    my-20 mx-auto 2xl:max-w-7xl px-5">
        <header class="flex flex-col space-y-2">
            <h1 class="2xl:text-3xl font-bold text-gray-700">Popular Projects in your
                Area(<span
                    class="uppercase text-main">{{ \App\Models\Location::where('zip_code', auth()->user()->client_information->zipcode ?? 1)->first()->location ?? '' }}</span>)
            </h1>
        ...
        <div class="mt-5 grid grid-cols-1 2xl:grid-cols-4 gap-5">
            @forelse ($categories as $item)
                <a href="/client/location/{{ $location_id }}/{{ $item->id }}"
                    class="border bg-white hover:shadow-main shadow-sm hover:shadow-lg rounded-xl ">
            ...
            @empty
                <span class="text-xl"> No Service Available in your area</span>
            @endforelse
        </div>
    </section>
    {{-- <x-modal.card title="Client Details" blur wire:model.defer="complete_details" align=center :clickAwayToClose="false">
        <div x-data="{
            bannerVisible: false,
    ...
    <div x-show="info" x-cloak class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <!--
          ...
          Leaving: "ease-in duration-200"
            ...
            To: "opacity-0"
        -->
        <div x-show="info" x-cloak class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
                            ...
                            <span class="flex items-center">
                                <svg class="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <g fill="none" stroke="none">
                                        ...
                                        <path d="M5 6.25V3.75M3.75 5H6.25" stroke="currentColor" stroke-width="1.5"
                                            stroke-linecap="round" stroke-linejoin="round"></path>
                                        <path d="M18 20.25V17.75M16.75 19H19.25" stroke="currentColor"
            ...
            -->
                <div x-show="info" x-cloak
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <header class="text-xl font-bold text-teal-800">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <x-button positive label="Submit Details" class="font-bold" wire:click="saveDetails" />
                        {{-- <x-button flat label="Cancel" rounded x-on:click="close" /> --}}
                    </div>
                </div>
            </div>
